...
const outputField = document.getElementById("output"); // Define outputField variable

translateButton.addEventListener("click", () => {
  const inputText = inputField.value.trim().toUpperCase();
  if (inputText === "") {
    outputField.value = "No Input Provided";
    return;
  }

  // Check if input contains only valid Morse code characters
  const isValidMorseCode = /^[./\sA-Z0-9-]*$/.test(inputText);
  if (!isValidMorseCode) {
